...
export const SearchBar = ({ setSearchTerm , visible }) => {
  ...
  return (
    <div className= {`bg-neutral-900  w-full rounded-2xl px-4 py-4 md:py-7 md:px-10 transition-all duration-1000 ease-in ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-7"} `}>
      <div className='flex items-center justify-between'>
        ...
        <div className='mt-5 sm:mt-8 w-full flex items-center'>
            <input onChange={(e)=>setSearchInput(e.target.value)} value={searchInput} className='w-full sm:flex-1 bg-neutral-900 border-2 border-neutral-600 rounded-full text-white py-2 px-4 sm:py-3 sm:px-5 md:text-xl' placeholder='Search' type="text"/>
